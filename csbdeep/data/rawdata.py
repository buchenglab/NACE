...
class RawData(namedtuple('RawData' ,('generator' ,'size' ,'description'))):
    ...
    @staticmethod
    def from_folder_txt(basepath, source_dirs, target_dir, axes='CZYX', pattern='*.txt*'):
        """Get pairs of corresponding TXT images read from folders.
        ...
        axes : str
            Semantics of axes of loaded images (assumed to be the same for all images).
        pattern : str
            Glob-style pattern to match the desired TXT images.

        Returns
        -------
        RawData
            :obj:`RawData` object, whose `generator` is used to yield all matching TXT pairs.
            The generator will return a tuple `(x,y,axes,mask)`, where `x` is from
            `source_dirs` and `y` is the corresponding image from the `target_dir`;
            `mask` is set to `None`.
        ...
        def _gen():
            for fx, fy in xy_name_pairs:
                x, y = loadtxt(str(fx)), imread(str(fy))
                # x.shape == y.shape or _raise(ValueError())
                len(axes) >= x.ndim or _raise(ValueError())
                yield x, y, axes[-x.ndim:], None
